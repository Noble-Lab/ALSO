...
printUsage() {
    ...
    echo "-o <\"lifted\" bed outfile, including path (chr)>"
    echo "-c <gzipped liftOver chain file including path (chr)>"
    exit
}

while getopts "h:u:i:o:c:" opt; do
    case "${opt}" in
        h) printUsage ;;
        u) safe_mode="${OPTARG}" ;;
        i) infile="${OPTARG}" ;;
...
[[ -z "${safe_mode}" ]] && safe_mode="TRUE"
[[ -z "${infile}" ]] && infile="/Users/kalavattam/Dropbox/UW/projects-etc/2021_kga0_4dn-mouse-cross/data/kga0/tbl.CAST.pos.chrX.bed"
[[ -z "${outfile}" ]] && 
