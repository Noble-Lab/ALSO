...
retain_qname_reads_samtools() {
    ...
    # write the filtered results to a bam outfile
    #
    # :param 1: number of threads for parallelization [int > 0]
    # :param 2: name of bam infile, including path [chr]
    # :param 3: name of txt QNAME list, including path [chr]
    # :param 4: name of bam outfile, including path (cannot be same as bam
    #           infile) [chr]
    start="$(date +%s)"

    samtools view -@ "${1}" "${2}" -N "${3}" -o "${4}"
