...
operation <- makeOperation(variable, command)
eval(parse(text = operation))
dedup.129S1 %>% nrow()  # [1] 494694 (chr1); [1] 274025 (chrX)
dedup.CAST %>% nrow()  # [1] 492644 (chr1); [1] 281850 (chrX)
dedup.mm10 %>% nrow()  # [1] 491416 (chr1); [1] 283022 (chrX)
...
command <- paste0("saveRDS(", variable[1:2], ", file = '", variable[1:2], ".", chromosome, ".liftOvers.rds')")
eval(parse(text = command))
...
#TODO 1/2 This should be the start of a new script here that loads in the saved
#TODO 2/2 .rds files
...
command <- paste0("<- ", s.variable, " %>% dplyr::select(coordinate, tag.AS) %>% dplyr::rename(AS.", suffix, " = tag.AS)")
