...
print_usage() {
    ...
    echo "-c  count lines: TRUE or FALSE [logical; default: TRUE]"
    exit
}
...
if [[ ! -f "${step_1}" ]]; then
    echo -e "Started step 1/5:" \
    "Sorting ${infile_1} and" "${infile_2}."
    
    sort_file_AS_overwrite_infile "${infile_1}" && \
    sort_file_AS_overwrite_infile "${infile_2}" && \
    touch "${step_1}"

    echo -e "Completed step 1/5:" \
    "Sorting ${infile_1} and" "${infile_2}."
else
...
if [[ ! -f "${step_2}" && -f "${step_1}" ]]; then
    echo -e "Started step 2/5:" \
    "Finding set intersection between ${infile_1} and ${infile_2}."

    find_set_intersection "${infile_1}" "${infile_2}" "${inter}" && \
    touch "${step_2}"

    echo -e "Completed step 2/5:" \
    "Finding set intersection between ${infile_1} and ${infile_2}.\n"
...
if [[ ! -f "${step_3}" && -f "${step_2}" ]]; then
    echo -e "Started step 3/5:" \
    "Between ${infile_1} and ${infile_2}, finding set complement for" \
    "${infile_1}."

    find_set_complement "${infile_1}" "${infile_2}" "${comp_1}" && \
    touch "${step_3}"

    echo -e "Completed step 3/5:" \
    "Between ${infile_1} and ${infile_2}, finding set complement for" \
    "${infile_1}.\n"
...
if [[ "${count}" == 1 ]]; then
    if [[ ! -f "${step_5}" && -f "${step_4}" ]]; then
        ...
        echo -e "Started step 5/5:" \
        "Check that the set intersection and set complement sums to the set" \
        "size."

        echo "Sum of $(basename "${inter}") and $(basename "${comp_1}"): $(( n_inter + n_comp_1 ))" && \
        echo "Count for $(basename "${infile_1}"): $(( n_1 ))" && \
        echo "" && \
        echo "Sum of $(basename "${inter}") and $(basename "${comp_2}"): $(( n_inter + n_comp_2 ))" && \
        echo "Count for $(basename "${infile_2}"): $(( n_2 ))" && \
        touch "${step_5}"

        echo -e "Completed step 5/5:" \
        "Check that the set intersection and set complement sums to the set" \
        "size.\n"
