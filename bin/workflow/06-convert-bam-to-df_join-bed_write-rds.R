...
makeOperation <- function(x, y) {
    # Make a string to be evaluated as an R operation
    # 
...
mungeMatesIntoOneRow <- function(x, y) {
    ...
    z <- full_join(odd, even, by = "groupid") %>%
        dplyr::arrange(y) %>%  # Sort by pos.x
...
#  Parse arguments ------------------------------------------------------------
script <- "convert-bam-to-df_join-bed_write-rds.R"
...
#  Join the POS and MPOS bed files to the tibble ------------------------------
pos <- readr::read_tsv(
    arguments$pos,
    col_names = c(
...
#  Keep only pairs with successful liftOver status and matching chromosomes ---
retain <- join %>%
    subset(
...
#  Write out rds file for munged pairs ----------------------------------------
