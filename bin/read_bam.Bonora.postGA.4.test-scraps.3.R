...
command <- paste0(
    "<- full_join(", variable_KA, ", joint.GB %>% select(-ID.GB, -read.GB, -tag.XS.GB, -tag.NM.GB), by = \"coordinate\") %>% tidyr::drop_na(\"assignment\")"
)
operation <- makeOperation(variable_joint, command)
eval(parse(text = operation))

command <- paste0(
    variable_joint, "$assignment_GB[is.na(", variable_joint, "$assignment_GB)] <- \"GB.not_present\""
)
eval(parse(text = operation))
...
command <- paste0("<- forcats::fct_relevel(", variable_KA, "$GB_intersection, order_intersection)")
operation <- makeOperation(paste0(variable_KA, "$GB_intersection"), command)
eval(parse(text = operation))

#  Copy the vector of binary names to a new variable; then, give them easier-
#+ to-read names 
command <- paste0("<- ", variable_KA, "$GB_intersection")
operation <- makeOperation(paste0(variable_KA, "$GB_assignment"), command)
eval(parse(text = operation))

command <- paste0(
    "<- ",
    variable_KA,
    "$GB_assignment %>% plyr::revalue(c(",
    "'0000' = 'GB.Not_present', ",
    "'0001' = 'GB.Contra', ",
    "'0010' = 'GB.Ambiguous', ",
    "'0100' = 'GB.CAST', ",
...
command <- paste0(
    "<- ", variable_KA, "$assignment %>% plyr::revalue(c(",
...
operation <- makeOperation(paste0(variable_KA, "$KA_assignment"), command)
eval(parse(text = operation))  # The errors reported here are fine...


#  Set up KA × GB assignment factors, KA × GB assignment factors
KA.129S1$KA_GB_assignment <- paste0(KA.129S1$KA_assignment, ' × ', KA.129S1$GB_assignment)
KA.CAST$KA_GB_assignment <- paste0(KA.CAST$KA_assignment, ' × ', KA.CAST$GB_assignment)
KA.NA$KA_GB_assignment <- paste0(KA.NA$KA_assignment, ' × ', KA.NA$GB_assignment)
KA.129S1$KA_GB_assignment <- paste0(KA.129S1$KA_assignment, ' × ', KA.129S1$GB_assignment)

KA.129S1$GB_KA_assignment <- paste0(KA.129S1$GB_assignment, ' × ', KA.129S1$KA_assignment)
KA.CAST$GB_KA_assignment <- paste0(KA.CAST$GB_assignment, ' × ', KA.CAST$KA_assignment)
KA.NA$GB_KA_assignment <- paste0(KA.NA$GB_assignment, ' × ', KA.NA$KA_assignment)
KA.129S1$GB_KA_assignment <- paste0(KA.129S1$GB_assignment, ' × ', KA.129S1$KA_assignment)

command <- paste0("<- ", variable_KA, "$KA_GB_assignment %>% as_factor()")
operation <- makeOperation(paste0(variable_KA, "$KA_GB_assignment"), command)
eval(parse(text = operation))  # Errors, but it still works...

command <- paste0("<- ", variable_KA, "$GB_KA_assignment %>% as_factor()")
operation <- makeOperation(paste0(variable_KA, "$GB_KA_assignment"), command)
eval(parse(text = operation))  # Errors, but it still works...
...
#TODO 1/3 Skip these analyses for now b/c need to go back and store liftOver,
#TODO 2/3 rname, pos, etc. info for both 129S1 and CAST; right now, only have
#TODO 3/3 info for 129S1
