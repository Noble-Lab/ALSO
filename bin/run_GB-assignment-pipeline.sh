...
cp \
"processed.mate-paired.129.chrX.bam" \
...
#TODO Get "${chromosome}" working in my call to Perl rename...
# find . -type f -name "*.bam" -print0 | xargs -0 rename STRING="${chromosome}" -n "s/.bam/\Q$ENV{STRING}\E.bam/g"
find . -type f -name "*.bam" -print0 | \
xargs -0 rename "s/segregated_reads.thresh_SNP_1.thresh_Q_30./segregated_reads.thresh_SNP_1.thresh_Q_30.chrX./g"
