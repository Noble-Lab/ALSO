...
#  Ignore the shell errors for tbl.mm10.*
...
#  Munge the information in the "unlifted" 129 and CAST variables -----------
...
tbl.129$liftOver_reason <- plyr::revalue(
    tbl.129$liftOver_reason,
    c(
...
test <- tbl.129[is.na(tbl.129$lO_reason), ]
test.dup <- test %>%
...
rm(test, test.dup)
#NOTE 1/2 Bug arises from running the '#  Munge the "unlifted" data' code chunk
#NOTE 2/2 more than once
#TODO Write a check: If not 0, then stop the program and print a warning


#  Clean up both CAST and 129 -----------------------------------------------
rm(
    chromosome,
    command,
    file,
...
# #  Check
# tbl.129$pos %>% is.na() %>% table(useNA = "ifany")
# tbl.129$mpos %>% is.na() %>% table(useNA = "ifany")
# tbl.CAST$pos %>% is.na() %>% table(useNA = "ifany")
# tbl.CAST$mpos %>% is.na() %>% table(useNA = "ifany")
